...
if os.path.isdir(sys.argv[1]):
    ...
    if False:
        pass
    elif 'inp.' not in sys.argv[1]:
        par25list=open(sys.argv[1],'r').readline().split()#;print(len(par25list));exit()
        if len(par25list)==26: par25list.pop(0)#;print(par25list);exit()
        Pi2PDG=( 61,62,0 ,65,63
			    ,64,43,46,49,33
			    ,36,11,12,13,42
				,45,48,32,35,1
				,2 ,3 ,124,125,16
			   )
        for i in range(len(par25list)):
            PDG=Pi2PDG[i]
            value=float(par25list[i])
            if PDG in free.EXTPAR.keys():
                free.EXTPAR[PDG].value=value
        free.MINPAR[3].value=float(par25list[2])
        for name in free.VariableList:
            print(name,getattr(free,name).value)
        r=N.run(free,attr='value')
        N.record(record)
        exit()
