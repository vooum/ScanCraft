...
from ..file_operations.container import capsule #Added by Mel 
...
class SPheno():
    ...
    def __init__(self,
            package_dir=None,
            input_mold='LesHouches.in.NMSSM_lowmel',   # Added by Mel
            input_file='LesHouches.in.NMSSM_lowmel',
            output_file='SPheno.spc.NMSSM',
            main_routine='SPhenoNMSSM',
            CleanRecord=None
        ):
        if package_dir==None:
            package_dir=GetPackageDir('SPheno_4.0.3') # Added by Mel
            print(package_dir)
        ...
        self.input_mold_lines=[SLHA_string(line) for line in open(os.path.join(package_dir,input_mold)).readlines()]
    ...
    def Record(self,number : int,digit_width:int=8):
        ...
        # shutil.copy(os.path.join(self.run_dir,'SPheno.spc.NMSSM'),self.record_dir)
        # shutil.copy(os.path.join(self.run_dir,'BR_H_NP.dat'),self.record_dir)
        # shutil.copy(os.path.join(self.run_dir,'BR_Hplus.dat'),self.record_dir)
        # shutil.copy(os.path.join(self.run_dir,'BR_t.dat'),self.record_dir)
        # shutil.copy(os.path.join(self.run_dir,'effC.dat'),self.record_dir)
        # shutil.copy(os.path.join(self.run_dir,'LEP_HpHm_CS_ratios.dat'),self.record_dir)
        # shutil.copy(os.path.join(self.run_dir,'MH_GammaTot.dat'),self.record_dir)
        # shutil.copy(os.path.join(self.run_dir,'MHplus_GammaTot.dat'),self.record_dir)
        documents={
